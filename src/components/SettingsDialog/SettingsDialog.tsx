...
import { type WorkspaceData, useWorkspaceStore } from "../../stores/workspaceStore";
...
function WorkspaceTab() {
  ...
  /** WorkspaceDataをパースしてタブ数を返す */
  const getTabCount = (dataStr: string): number => {
    try {
      const data: WorkspaceData = JSON.parse(dataStr);
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          />
          <button
            className="px-3 py-1.5 text-xs bg-[var(--accent)] hover:opacity-90 rounded text-white transition-colors disabled:opacity-50"
            onClick={handleSave}
        ...
        {workspaces.length === 0 ? (
          ...
          <div className="space-y-1.5 max-h-[240px] overflow-y-auto">
            {workspaces.map((ws) => (
              <div
                key={ws.id}
                className="flex items-center justify-between gap-2 px-3 py-2 rounded bg-[#fafafa] hover:bg-[#f0f0f0] transition-colors group"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <FolderOpen className="w-3.5 h-3.5 text-[#999] shrink-0" />
                    <span className="text-xs text-[#333] truncate font-medium">
                      {ws.name}
                    </span>
                    <span className="text-[10px] text-[#bbb] shrink-0">
                      {getTabCount(ws.data)} {t.workspace.tabs}
                    </span>
                  </div>
                  <div className="text-[10px] text-[#bbb] mt-0.5 ml-5">
                    {new Date(ws.updated_at).toLocaleString()}
                  </div>
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    className="px-2 py-1 text-[10px] bg-[var(--accent)] hover:opacity-90 rounded text-white transition-colors"
